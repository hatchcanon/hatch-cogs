...
class ForgeReplaceSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        await interaction.response.defer()
        self.view.replace_item = self.items[int(self.values[0])]
        self.view.confirmed = True
        self.view.stop()


class ForgeReplaceView(discord.ui.View):
    """Lets the user pick which of their existing forged items to replace with a new one."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(timeout=timeout)
        self.confirmed = None
        self.replace_item: Optional[Item] = None
        self.author = author
        self.message: Optional[discord.Message] = None
        self.item_name = None
        self.add_item(ForgeReplaceSelect(items))
        self.name_button = ForgeNameButton()
        if get_name:
            self.add_item(self.name_button)
