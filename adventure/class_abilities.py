...
from .helpers import ConfirmView, ForgeReplaceView, escape, is_dev, smart_embed
    ...
    async def forge(self, ctx: commands.Context):
        ...
        async with self.get_lock(ctx.author):
            ...
            if c.hc is not HeroClasses.tinkerer:
                ...
                if len(lookup) >= FORGE_LIMIT:
                    msg += box(
                        _("{author}, you already have {count} devices. Which one do you want to replace?").format(
                            author=escape(ctx.author.display_name),
                            count=len(lookup),
                        ),
                        lang="ansi",
                    )
                    view = ForgeReplaceView(60, ctx.author, lookup, get_name=True)
